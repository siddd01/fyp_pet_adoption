...
const StaffResetPassword = () => {
  ...
  const applyServerState = (data, fallbackMessage) => {
    ...
    setTokenExpired(normalized.otpExpired);
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      setBlockedUntil(null);
      setOtp("");
      setSuccessMsg(res.data?.message || "A new code has been sent to your staff email.");
    } catch (err) {
      applyServerState(err.response?.data, "Failed to resend code.");
    } finally {
      setResending(false);
    }
  };
  ...
  const handleVerifyStep = async (e) => {
    e.preventDefault();
    if (isBlocked || tokenExpired) return;
    ...
    try {
      await api.post("/staff/verify-reset-otp", { email, otp });
      setRemainingTries(MAX_OTP_TRIES);
      setBlockedUntil(null);
      setIsOtpVerified(true);
      setSuccessMsg("OTP verified. You can set a new staff password now.");
    } catch (err) {
  ...
  const handleResetStep = async (e) => {
    ...
    try {
      ...
      setSuccessMsg("Staff password updated successfully. Redirecting to login...");
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#ecfeff,_#fafaf9_45%,_#f5f5f4_100%)] px-6 py-10">
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-md items-center">
        <div className="w-full overflow-hidden rounded-[3rem] border border-stone-200 bg-white shadow-[0_30px_90px_-35px_rgba(28,25,23,0.35)]">
          <div className="bg-[linear-gradient(135deg,#111827_0%,#1f2937_50%,#0f766e_100%)] px-8 py-9 text-white">
            ...
            <p className="text-[10px] font-bold uppercase tracking-[0.32em] text-teal-100/80">Staff Security</p>
            <h2 className="mt-3 text-4xl leading-tight" style={{ fontFamily: "Georgia, serif" }}>
              {isOtpVerified ? "Create new password" : "Verify staff token"}
            </h2>
            <p className="mt-3 text-sm leading-6 text-stone-200">
              {isOtpVerified ? "Your code is approved. Finish by setting a fresh staff password." : `Use the 6-digit code sent to ${email}.`}
            </p>
          </div>

          <div className="space-y-6 p-8">
            {tokenExpired && (
              <div className="rounded-[1.6rem] border border-red-300 bg-[linear-gradient(135deg,#7f1d1d_0%,#dc2626_55%,#fb7185_100%)] px-5 py-5 text-white shadow-[0_24px_60px_-30px_rgba(220,38,38,0.95)]">
                <div className="flex items-start gap-3">
                  <AlertCircle className="mt-0.5 h-5 w-5 shrink-0" />
                  <div>
                    <p className="text-[11px] font-black uppercase tracking-[0.28em] text-red-100">Token Expired</p>
                    <p className="mt-2 text-sm leading-6 text-red-50">
                      Your reset token has expired. Redirecting you to staff login so you can start again securely.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleVerifyStep} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    maxLength={6}
                    disabled={loading || isBlocked || tokenExpired}
                    required
                  />
                </div>
                ...
                <button
                  ...
                  disabled={resending || isBlocked || tokenExpired}
