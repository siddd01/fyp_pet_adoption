...
const AdminResetPassword = () => {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      setBlockedUntil(null);
      setOtp("");
      setSuccessMsg(res.data?.message || "A new code has been sent to your admin email.");
    } catch (err) {
  ...
  const handleVerifyStep = async (e) => {
    ...
    try {
      ...
      setSuccessMsg("OTP verified. You can set a new admin password now.");
    } catch (err) {
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#fff1f2,_#fafaf9_45%,_#f5f5f4_100%)] px-6 py-10">
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-md items-center">
        <div className="w-full overflow-hidden rounded-[3rem] border border-stone-200 bg-white shadow-[0_30px_90px_-35px_rgba(28,25,23,0.35)]">
          <div className="bg-[linear-gradient(135deg,#111827_0%,#1f2937_50%,#7f1d1d_100%)] px-8 py-9 text-white">
            ...
            <p className="text-[10px] font-bold uppercase tracking-[0.32em] text-rose-100/80">Admin Security</p>
            <h2 className="mt-3 text-4xl leading-tight" style={{ fontFamily: "Georgia, serif" }}>
              {isOtpVerified ? "Create new password" : "Verify admin token"}
            </h2>
            <p className="mt-3 text-sm leading-6 text-stone-200">
              {isOtpVerified ? "Your code is approved. Finish by setting a fresh admin password." : `Use the 6-digit code sent to ${email}.`}
            </p>
          </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-6 text-red-50">
                      Your reset token has expired. Redirecting you to admin login so you can start again securely.
                    </p>
                  </div>
                </div>
              </div>
