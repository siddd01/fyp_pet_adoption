...
const Notifications = () => {
  ...
  const [visibleByTab, setVisibleByTab] = useState({
    adoptions: PAGE_SIZE,
    reports: PAGE_SIZE,
  });
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const nextReportNotifications = reportRes.data.notifications || [];
      setReportNotifications(nextReportNotifications);
    } catch {
      // Keep the page quiet and let the empty states handle the result.
    } finally {
      setLoading(false);
    }
  };
  ...
  const reportUnreadCount = reportNotifications.filter((notification) => !notification.is_read).length;

  const counts =
    activeTab === "adoptions"
      ? {
          all: adoptionNotifications.length,
          approved: adoptionNotifications.filter((notification) => notification.status === "approved").length,
          pending: adoptionNotifications.filter((notification) => notification.status === "pending").length,
          rejected: adoptionNotifications.filter((notification) => notification.status === "rejected").length,
        }
      : {
          all: reportNotifications.length,
          unread: reportNotifications.filter((notification) => !notification.is_read).length,
          read: reportNotifications.filter((notification) => notification.is_read).length,
        };
  ...
  const filtered =
    activeTab === "adoptions"
      ? filter === "all"
        ...
        : adoptionNotifications.filter((notification) => notification.status === filter)
      : filter === "all"
        ? reportNotifications
        : filter === "unread"
          ? reportNotifications.filter((notification) => !notification.is_read)
          : reportNotifications.filter((notification) => notification.is_read);

  const visibleItems = filtered.slice(0, visibleByTab[activeTab]);
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-12">
        {activeTab === "adoptions" ? (
          visibleItems.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-stone-200 bg-stone-50 px-6 py-16 text-center text-stone-500">
              No adoption updates found for this filter.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {visibleItems.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="group bg-white rounded-4xl border border-stone-100 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 flex flex-col overflow-hidden"
                  >
                    ...
                    <div className="p-8 flex flex-col h-full">
                      <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-4">
                          <img
                            src={item.image_url || DEFAULT_PET_IMAGE}
                            alt=""
                            className="w-16 h-16 rounded-2xl object-cover ring-4 ring-stone-50"
                          />
                          <div>
                            <h3
                              className="text-stone-900 font-semibold text-xl"
                              style={{ fontFamily: "Georgia, serif" }}
                            >
                              {item.pet_name}
                            </h3>
                            <p className="text-[10px] text-stone-400 uppercase tracking-tighter">
                              Application ID: #{item.id.toString().slice(-4)}
                            </p>
                          </div>
                        </div>
                        <span
                          className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase border ${cfg.bg} ${cfg.text} ${cfg.border}`}
                        >
                          {cfg.label}
                        </span>
                      </div>
                      ...
                      <div className="bg-stone-50 p-5 rounded-2xl mb-8">
                        <p className="text-[9px] text-stone-400 uppercase tracking-widest font-bold mb-2">
                          ...
                          {item.status === "pending" ? (
                            <>
                              <button
                                ...
                                className="bg-stone-900 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-stone-700 transition-colors shadow-lg shadow-stone-200"
                              >
                                Edit
                              </button>
                              <button
                                onClick={() => handleDelete(item.id)}
                                className="bg-red-600 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-red-700 transition-colors shadow-lg shadow-red-200"
                              >
                                Delete
                              </button>
                            </>
                            ...
                            <button
                              ...
                              className="text-stone-900 text-[10px] font-bold uppercase tracking-widest border-b border-stone-900/20 hover:border-stone-900 transition-all py-1"
                            >
                              View Details
                            </button>
                          )}
          ...
          <div className="max-w-4xl mx-auto rounded-3xl border border-dashed border-stone-200 bg-stone-50 px-6 py-16 text-center text-stone-500">
            No report updates found for this filter.
          </div>
        ) : (
        ...
        {hasMoreItems && (
          <div className="mt-10 flex justify-center">
            <button
              ...
              className="rounded-2xl border border-stone-300 bg-white px-6 py-3 text-xs font-bold uppercase tracking-[0.2em] text-stone-800 hover:bg-stone-100 transition"
            >
              See More
            </button>
          </div>
            ...
            <div className="p-10">
              <div className="flex justify-between items-center mb-10">
                <h2
                  ...
                  style={{ fontFamily: "Georgia, serif" }}
                >
                  Application <span className="italic text-stone-400">Log</span>
                </h2>
                <button
                  onClick={() => setSelectedApp(null)}
                  className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-stone-100 text-stone-400 transition-colors text-2xl"
                >
                  &times;
                </button>
              </div>
              ...
              <div className="space-y-8">
                <div className="flex gap-6 p-6 bg-stone-50 rounded-3xl border border-stone-100">
                  <img
                    src={selectedApp.image_url || DEFAULT_PET_IMAGE}
                    alt={selectedApp.pet_name}
                ...
                <div className="grid grid-cols-2 gap-y-8 gap-x-4">
                  ...
                  <DetailItem
                    ...
                    value={new Date(selectedApp.created_at).toLocaleDateString()}
                  />
                </div>
                ...
                <div className="pt-4 border-t border-stone-100 space-y-6">
                  <DetailItem
                    ...
                    value={selectedApp.experience_with_pets || "No previous experience listed."}
                  />
