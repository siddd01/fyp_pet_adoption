    ...
    <div className="min-h-screen bg-stone-50">
          ...
          <div className="space-y-8">
            ...
            {isLoggedIn && user ? (
              <div className="inline-flex items-center gap-2 bg-white border border-stone-200 rounded-full px-4 py-1.5 text-sm text-stone-600 shadow-sm">
                ...
                Welcome back,{" "}
                <span className="font-semibold text-stone-900">{user.first_name}</span> 👋
              </div>
            ) : (
      ...
      <div className="max-w-6xl mx-auto px-6 pb-16">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <p className="text-xs tracking-[0.25em] uppercase text-stone-400">What we offer</p>
          <div className="flex-1 h-px bg-stone-200" />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {features.map(({ emoji, label, sub, to }) => (
            <div
              key={label}
              onClick={() => navigate(to)}
              className="bg-white rounded-2xl border border-stone-100 shadow-sm px-6 py-5 flex items-center gap-4 hover:shadow-md hover:border-stone-200 hover:-translate-y-0.5 transition-all cursor-pointer group"
            >
              <div className="w-11 h-11 bg-stone-100 group-hover:bg-stone-200 transition rounded-xl flex items-center justify-center text-2xl shrink-0">
                {emoji}
              </div>
